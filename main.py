    ...
    if os.path.exists(JSON_FILE):
        with open(JSON_FILE, "r") as file:
            try:
                ...
                return []
    return [{"id": "demo-1", "title": "Welcome Note", "category": "Notes", "content_type": "text", "text_content": "Welcome to your minimalist repository portal!", "image_path": "", "date": "May 29, 2026"}]
    ...
    for item in class_materials:
        ...
        if image_path:
            ...
            <div class="mt-3 border border-slate-700/50 rounded-xl overflow-hidden bg-slate-900/40">
                <img src="{image_path}" class="w-full h-auto max-h-60 object-cover" alt="Notes Image">
            </div>
            """

        view_btn = ""
        if image_path:
            view_btn = f'<a href="{image_path}" target="_blank" class="text-xs bg-slate-800 border border-slate-700 text-indigo-400 hover:bg-slate-700/50 px-3 py-1.5 rounded-lg font-medium transition shrink-0">View</a>'
        elif text_content:
            view_btn = '<span class="text-xs text-slate-500 italic px-1 shrink-0">Text Only</span>'
        else:
            view_btn = '<span class="text-xs text-slate-500 italic px-1 shrink-0">Empty Log</span>'
        ...
        <div class="material-card bg-slate-900/60 backdrop-blur-md p-4 sm:p-5 rounded-2xl border border-slate-800/80 shadow-xl flex flex-col justify-between space-y-3" data-category="{category}">
            <div class="flex justify-between items-start gap-2">
                <div class="truncate flex-1">
                    ...
                    <p class="text-sm font-bold text-slate-100 truncate tracking-wide whitespace-normal break-words">{item.get('title') or 'Untitled Note'}</p>
                    <span class="text-[11px] text-slate-400 block mt-0.5 font-mono">{item.get('date') or 'No Timestamp'}</span>
                </div>
                ...
                <div class="bg-slate-900/40 border border-slate-800/80 p-4 sm:p-6 rounded-2xl shadow-2xl space-y-4 backdrop-blur-lg">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/60 pb-3">
                        <h3 class="text-xs font-extrabold uppercase tracking-widest text-indigo-400 flex items-center gap-2">
                            ...
                            Flexible Upload Terminal
                        </h3>
                        
                        <div class="flex gap-2 bg-slate-950 p-1 rounded-lg border border-slate-800 text-[10px] font-bold">
                            <label class="cursor-pointer px-2 py-1 rounded transition text-slate-300"><input type="radio" name="source_type" value="image" checked onclick="toggleInputType('image')" class="mr-1 accent-indigo-500">Photo Mode</label>
                            <label class="cursor-pointer px-2 py-1 rounded transition text-slate-300"><input type="radio" name="source_type" value="text" onclick="toggleInputType('text')" class="mr-1 accent-indigo-500">Text / Link Mode</label>
                        </div>
                    </div>
                    ...
                    <form action="/upload-note" method="POST" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div class="md:col-span-2">
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Document Title (Optional)</label>
                                <input type="text" name="title" placeholder="Leave blank or type title..." class="w-full text-xs p-2.5 rounded-xl border border-slate-800 bg-slate-950 text-slate-100 placeholder:text-slate-600 focus:outline-none focus:border-indigo-500 transition">
                            </div>
                            <div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div>
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Timestamp Date (Optional)</label>
                                <input type="text" name="date" placeholder="e.g. 29 May 2026" class="w-full text-xs p-2.5 rounded-xl border border-slate-800 bg-slate-950 text-slate-100 focus:outline-none focus:border-indigo-500 transition">
                            </div>
                            
                            <div class="sm:col-span-2" id="file-input-wrapper">
                                <label class="block text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1">Select File (Optional)</label>
                                <input type="file" name="file" id="file_input" class="w-full text-xs text-slate-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-xl file:border-0 file:text-[11px] file:font-bold file:bg-indigo-600 file:text-white hover:file:bg-indigo-700 cursor-pointer">
                            </div>
                            
                            <div class="sm:col-span-2 hidden" id="text-input-wrapper">
                                <label class="block text-[10px] font-bold text-indigo-400 uppercase tracking-wider mb-1">Text Notes / Links (Optional)</label>
                                <input type="text" name="text_content" id="text_content_input" placeholder="Type data or paste link here..." class="w-full text-xs p-2.5 rounded-xl border border-slate-800 bg-slate-950 text-slate-100 focus:outline-none focus:border-indigo-500 transition">
                            </div>
                        </div>
                        
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end pt-2 border-t border-slate-800/50">
                            <div class="sm:col-span-2">
                                <label class="block text-[10px] font-bold text-amber-400 uppercase tracking-wider mb-1">🔑 Admin Access Token (Password) - REQUIRED</label>
                                <input type="password" name="password" mercantile required placeholder="Enter Secret Code to Authenticate" class="w-full text-xs p-2.5 rounded-xl border border-amber-500/30 bg-slate-950 text-slate-100 focus:outline-none focus:border-amber-500 transition">
                            </div>
                            <div>
...
def upload_note_with_image(
    ...
    # Check if a physical image was uploaded
    ...
    if text_content and text_content.strip():
        # If text is present, we enforce it as text type (or it can coexist with image)
